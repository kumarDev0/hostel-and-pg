...
export default function About({ config }: { config: ClientConfig }) {
  const amenities = [
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative">
            <img src={config.aboutImage} alt={config.hostelName} className="w-full h-[600px] object-cover object-center rounded-[2rem] shadow-xl" referrerPolicy="no-referrer" />
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-ink/70 mb-8 leading-relaxed text-lg font-light">
              {config.hostelName} is designed to be a sanctuary in the bustling city. We believe in fostering connections, sharing stories, and providing a comfortable environment where you can truly unwind.
            </p>
